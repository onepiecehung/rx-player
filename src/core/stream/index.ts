...
import getInitialTime from "./initial_time";
import {
...
export default function Stream({
   ...
   * @returns {Observable}
   */
  const startStream = retryableFuncWithBackoff(({ url: _url, mediaSource }) => {
    const sourceOpening$ = mediaSource
      ? onSourceOpen$(mediaSource)
      : Observable.of(null);

    return Observable.combineLatest(fetchManifest(_url), sourceOpening$)
      ...
      const abrClock$ = timings$
        .map(timing => {
          ...
          if (currentRepresentation) {
            const _currentRepresentation = currentRepresentation as any;
            bitrate = _currentRepresentation.bitrate;

            if (_currentRepresentation.index) {
              lastIndexPosition =
                _currentRepresentation.index.getLastPosition();
            }
          }
