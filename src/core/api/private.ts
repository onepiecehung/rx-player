...
export default (self) => ({
   ...
   */
  recordState(type : string, value : any) : void {
  ...
  onStreamNext(streamInfos) : void {
    ...
    case "pipeline":
      ...
      if (bufferType === "image") {
        ...
        self._priv.currentImagePlaylist = segmentData;
  ...
  onManifestChange(value) : void {
    ...
    const { manifest, adaptations$ = {} }
      : { manifest : any, adaptations$ : any } = value;
   ...
   */
  onAdaptationChange({ type, adaptation }) : void {
   ...
   */
  onRepresentationChange({ type, representation }) : void {
  ...
  onBitrateEstimationChange({ type, bitrate }) : void {
    if (__DEV__) {
      assert(type != null);
      assert(bitrate != null);
    }
   ...
   */
  onPlayPauseNext(isPlaying : boolean) : void {
   ...
   */
  onNativeTextTracksNext(tracks : TextTrack[]) : void {
