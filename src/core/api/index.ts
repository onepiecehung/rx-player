...
import attachPrivateMethods from "./private";
import inferPlayerState from "./infer_player_state";
import {
 ...
 * @returns {Observable}
 */
function filterStreamByType<T>(
  stream : Observable<{ type : string, value : T }>,
  type : string
) : Observable<T> {
  return stream
...
class Player extends EventEmitter {
  ...
  public log : ILogger;
  public state? : string;
  public defaultTransport? : string;
  ...
  constructor(options) {
    ...
    onTextTrackChanges$(videoElement.textTracks)
      .takeUntil(this._priv.destroy$)
      .map((evt) => { // prepare TextTrack array
  ...
  loadVideo(options : any = {}) : void {
    ...
    assert(this.videoElement, "the attached video element is disposed");
    ...
    assert(transportFn, `transport "${transport}" not supported`);
    ...
    const videoElement = this.videoElement as HTMLMediaElement;
    const {
      errorStream$,
    ...
    unsubscribeLoadedVideo$.take(1).subscribe(() => {
      if (streamDisposable) {
        (streamDisposable as any).unsubscribe();
      }
    });
  ...
  getVideoDuration() : number {
    return (this.videoElement as HTMLMediaElement).duration;
  }
  ...
  getWallClockTime() : number {
    ...
    const ct = (this.videoElement as HTMLMediaElement).currentTime;
    return this.isLive() ?
   ...
   */
  getPosition() : number {
    return (this.videoElement as HTMLMediaElement).currentTime;
  }
   ...
   */
  getVolume() : number {
    return (this.videoElement as HTMLMediaElement).volume;
  }
  ...
  seekTo(time : number|any) : number {
    ...
    if (type === "number") {
      ...
      const currentTs = (this.videoElement as HTMLMediaElement).currentTime;
    ...
    (this.videoElement as HTMLMediaElement).currentTime = positionWanted;
  ...
  setFullscreen(goFull : boolean = true) : void {
    if (goFull) {
      requestFullscreen(this.videoElement as HTMLMediaElement);
    } else {
      exitFullscreen();
    }
  }
