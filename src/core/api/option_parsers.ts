...
import takeFirstDefined from "../../utils/takeFirstDefined";
import {
  normalizeAudioTrack,
  normalizeTextTrack,
 ...
 * @returns {Object}
 */
function parseConstructorOptions(options : any = {}) {
  const parsed : any = {
  ...
  const defaultInitialBitrates : any = DEFAULT_INITIAL_BITRATES || {};
  const defaultMaxBitrates : any = DEFAULT_MAX_BITRATES || {};
 ...
 * @returns {Object}
 */
function parseLoadVideoOptions(options : any = {}, ctx) {
  const {
    defaultTransport,
