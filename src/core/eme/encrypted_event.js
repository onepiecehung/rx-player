...
import setMediaKeysObs from "./set_media_keys.js";
import manageSessionCreation from "./session.js";
import findCompatibleKeySystem from "./key_system.js";
 ...
 * @returns {Observable}
 */
function onEncryptedEvent(encryptedEvent, {
  keySystem,
  keySystemAccess,
}, video, errorStream) {
 ...
 * @returns {Observable}
 */
export default function handleEncryption(video, keySystems, errorStream) {
  if (__DEV__) {
  ...
  return Observable.combineLatest(
    ...
    .take(1)
    .mergeMap(([evt, ks]) => {
      return onEncryptedEvent(evt, ks, video, errorStream);
    });
}
