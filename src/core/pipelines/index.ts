 ...
 * @returns {Error}
 */
function errorSelector(code, error, fatal : boolean = true) {
...
export default function createPipeline(
  ...
  const _loader = (resolvedInfos, pipelineInputData) => {
    const loaderWithRetry = (_resolvedInfos) =>
      // TODO do something about bufferdepth to avoid infinite errors?
      downloadingBackoff(tryCatch(loader, _resolvedInfos), backoffOptions)
        .catch((error) => {
  ...
  return (pipelineInputData) => {
    const pipeline$ = _resolver(pipelineInputData)
      ...
      }).do(null, null, () => { retryErrorSubject.complete(); });
