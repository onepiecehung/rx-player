...
function compatibleListener(
  eventNames : string[],
  prefixes? : string[]
) : (element : Element) => Observable<Event> {
...
function visibilityChange() : Observable<boolean> {
  ...
  if (document.hidden != null) {
    prefix = "";
  } else if ((document as any).mozHidden != null) {
    prefix = "moz";
  } else if ((document as any).msHidden != null) {
    prefix = "ms";
  } else if ((document as any).webkitHidden != null) {
    prefix = "webkit";
  }
  ...
  return onEvent(document, visibilityChangeEvent)
    .map(() => (document as any)[hidden] as boolean);
}
