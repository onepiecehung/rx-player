...
function isCodecSupported(codec : string) : boolean {
  ...
  // TODO add to TypeScript?
  if (typeof (MediaSource_ as any).isTypeSupported === "function") {
    return (MediaSource_ as any).isTypeSupported(codec);
  }
 ...
 * @returns {Boolean}
 */
function isVTTSupported() : boolean {
  return !isIE;
}
