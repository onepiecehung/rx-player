...
import Adaptation, { IAdaptationArguments } from "./adaptation";
...
class Manifest {
  ...
  constructor(args : IManifestArguments) {
    ...
    this.adaptations = Object.keys(args.adaptations).reduce((
      acc : IManifestAdaptations,
      val : "text"|"audio"|"video"|"image"
    ) => {
      acc[val] = (args.adaptations[val] || [])
        .map((a : any) => new Adaptation(a));
      return acc;
    }, {}) || [];
  ...
  getAdaptations() : Adaptation[] {
    ...
    const adaptationsList : any[] = [];
    for (const type in adaptationsByType) {
      if (adaptationsByType.hasOwnProperty(type)) {
  ...
  getAdaptation(wantedId : number|string) : Adaptation|undefined {
    return arrayFind(this.getAdaptations(),
      ({ id } : Adaptation) => wantedId === id);
  }
